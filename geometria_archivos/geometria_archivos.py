...
class cuadrado:
    ...
    def codificar(self):
        return "cuadrado "+str(self.x)+" "+str(self.y)+" "+str(self.lado)+"\n"
...
class circulo:
    ...
    def codificar(self):
        return "circulo "+str(self.x)+" "+str(self.y)+" "+str(self.radio)+"\n"
...
class lienzo:
    ...
    def guardar(self, filename):
        f=open(filename,'w')
        for obj in self.objetos:
            f.write(obj.codificar()) #obj codificar retorna la codificación del objeto
        f.close()
    ...
    def leer(self, filename):
        ...
        for linea in f:
            l = linea.split()
            if l[0]=="circulo":
                obj=circulo(int(l[1]),int(l[2]),float(l[3]))
            elif l[0]=="cuadrado":
                obj=cuadrado(int(l[1]),int(l[2]),float(l[3]))
            self.agregar(obj)
        f.close()
...
def pregunta_circulo():
    ...
    while True:
        ...
        if len(l) == 3 and l[0].isnumeric() and l[1].isnumeric() and l[2].isnumeric():
            break
        print("Ingresa un input legal de tres numeros!")
    radio=int(l[0])
    x = int(l[1])
    y = int(l[2])
    circ = circulo(x,y,radio)
    return circ
